...
const Energy = () => {
  ...
  const gestureOutput  =
    document.getElementById("gesture_output");
  ...
  async function predictWebcam() {
    ...
    if (results && results.gestures.length > 0 && gestureOutput) {
      ...
      const landeightx  = parseFloat(
        results.landmarks[0][7].x * 2
      ).toFixed(2);
      const landeighty  = parseFloat(
        results.landmarks[0][7].y * 2
      ).toFixed(2);
      const landeightz  = parseFloat(
        results.landmarks[0][7].z * 100
      ).toFixed(2);
